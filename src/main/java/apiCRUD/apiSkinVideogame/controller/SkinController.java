...
public class SkinController {
    ...
    public ResponseEntity<?> getMySkins() {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        return new ResponseEntity<>("User with ID " + user.getId() + " has this skins: " + skinService.getMySkins(user), HttpStatus.OK);
    }

    @PutMapping("{skinID}/{color}")
    @PreAuthorize("hasRole('USER') or hasRole('ADMIN')")
    public ResponseEntity<?> changeSkinColor(@PathVariable(value = "userID") Long userID, @PathVariable(value = "skinID") Long skinID, @PathVariable(value = "color") String color) {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        return new ResponseEntity<>(skinService.changeSkinColor(user, skinID, color), HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> deleteSkinById(@PathVariable(value = "skinID") Long skinID) throws IOException {
        User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        return new ResponseEntity<>(skinService.deleteSkinByUser(skinID, user), HttpStatus.OK);
